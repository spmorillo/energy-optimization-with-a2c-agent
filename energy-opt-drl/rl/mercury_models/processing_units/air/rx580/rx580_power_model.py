...
class Rx580PowerModel(ProcessingUnitPowerModel):
    def __init__(self, **kwargs):
        ...
        with open(dirpath + 'model.json', 'r') as f:
            self.model = model_from_json(f.read())
        self.model.load_weights(dirpath + 'weights.h5')
        self.scaler_x = load_scaler(dirpath + 'scaler_x.save')
        self.scaler_y = load_scaler(dirpath + 'scaler_y.save')
    ...
    def compute_power(self, status, utilization, max_u, dvfs_index, dvfs_table):  # TODO con max_u
        ...
        if status:
            ...
            x[0, 2] = (0.8 * utilization) / 20.0  # TODO esto es lo raro
